...
for ts in all_ts:
    ...
    interact_19 = pd.concat([interact_19, append_row]) 


y_min = interact_19['Vehicle Speed'].min() - 0.1
y_max = interact_19['Vehicle Speed'].max() + 0.1

new_interact_19 = interact_19.dropna(how='any')
new_interact_19.plot(x="Timestamp", y=["Pedestrian Speed", "Vehicle Speed"], kind="bar")
plt.title('Speed: Both Vehicle and Pedestrian Present at a TS')
plt.ylim(y_min, y_max)

new_interact_19 = interact_19.dropna(how='any').head(10)
new_interact_19.plot(x="Timestamp", y=["Pedestrian Speed", "Vehicle Speed"], kind="bar")
plt.title('Speed: only first 10 rows When both Vehicle and Pedestrian Present at a TS')
plt.ylim(y_min, y_max)

new_interact_19 = interact_19.head(10)
new_interact_19.plot(x="Timestamp", y=["Pedestrian Speed", "Vehicle Speed"], kind="bar")
plt.title('Speed: only first 10')
plt.ylim(y_min, y_max)

new_interact_19 = interact_19[interact_19['interaction'] == 1]
new_interact_19.plot(x="Timestamp", y=["Pedestrian Speed", "Vehicle Speed"], kind="bar")
plt.title('Speed: Interacted row')
plt.ylim(y_min, y_max)


y_min = interact_19['distance'].min() - 1
y_max = interact_19['distance'].max() + 5

new_interact_19 = interact_19.dropna(how='any')
new_interact_19.plot(x="Timestamp", y=["distance"], kind="bar")
plt.title('Distance: When both Vehicle and Pedestrian Present at a Timestamp')
plt.ylim(y_min, y_max)

new_interact_19 = interact_19.dropna(how='any').head(35)
new_interact_19.plot(x="Timestamp", y=["distance"], kind="bar")
plt.title('Distance: only first 35 rows When both Vehicle and Pedestrian Present at a Timestamp')
plt.ylim(y_min, y_max)

new_interact_19 = interact_19[interact_19['interaction'] == 1]
new_interact_19.plot(x="Timestamp", y=["distance"], kind="bar")
plt.title('Distance: Interacted row')
plt.ylim(y_min, y_max)
